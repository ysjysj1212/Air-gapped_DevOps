...
"""
    ...
    def test_gitlab_safety_blocks_network_commands(self):
        """Test GitLab safety inspection blocks dangerous network commands"""
        yaml_content = """stages:
  - validate
image: node:20
validate:
...
"""
        result = inspect_gitlab_safety(yaml_content)
        assert result.is_safe is False
        assert any("curl" in item["command"] for item in result.blocked_commands)

    def test_gitlab_safety_warns_for_unsupported_image(self):
        """Test GitLab safety inspection warns when sandbox cannot verify an image"""
        yaml_content = """stages:
  - validate
image: golang:1.22
validate:
...
"""
        ...
        assert result.sandbox_compatible is False
        assert any("sandbox-unsupported image" in warning for warning in result.warnings)
